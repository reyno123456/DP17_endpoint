...
void READ_ADS1110(void)
{
    //volatile u8 TMTEMP;
    IIC_Start();
    IIC_Send_Byte(0x91) ;//读地址
    IIC_Wait_Ack() ; 
    ADS_DATA.TMdata = IIC_Read_Byte(1)<<8;
    IIC_Ack(); //
    ADS_DATA.TMdata += IIC_Read_Byte(1);//读出的转换数据存入16位变量中
    IIC_Ack(); //
    ADS_DATA.TMTEMP = IIC_Read_Byte(1);
    IIC_Ack(); //
    IIC_Stop();
}
